...
export const USERS_DEAD_QUEUE = 'users.dead.q';
...
export async function declareTopology(channel: Channel): Promise<void> {
  ...
  await channel.assertQueue(USERS_DEAD_QUEUE, { durable: true });
  await channel.bindQueue(USERS_DEAD_QUEUE, DOMAIN_EVENTS_DLX, '#');
}
