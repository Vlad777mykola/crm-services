...
export async function consumeFromRabbitMq(options: ConsumeOptions): Promise<RabbitMqConsumer> {
  ...
  connection.on('error', (err: unknown) => logger.warn({ err }, '[users-service] RabbitMQ connection error'));
...
async function handleMessage(
  ...
  try {
    ...
    logger.error(
      ...
      '[users-service] failed to process message - routing to dead-letter queue',
    );
