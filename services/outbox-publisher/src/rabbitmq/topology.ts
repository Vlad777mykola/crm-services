...
export const OUTBOX_DEAD_QUEUE = 'outbox.dead.q';
...
export async function declareTopology(channel: Channel): Promise<void> {
  ...
  // This service's own dead letters: an event it could never publish after
  // MAX_ATTEMPTS ends up here for manual inspection instead of being lost.
  await channel.assertQueue(OUTBOX_DEAD_QUEUE, { durable: true });
  await channel.bindQueue(OUTBOX_DEAD_QUEUE, DOMAIN_EVENTS_DLX, '#');
}
