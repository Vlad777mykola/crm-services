...
export async function processInboundEvent(deps: ProcessInboundEventDeps, envelope: InboundEnvelope): Promise<void> {
  await deps.dataSource.transaction(async (manager) => {
    ...
    if (!isNewEvent) {
      logger.info({ eventId: envelope.id }, '[company-specialists-service] already processed - skipping');
      return;
    }
    ...
    logger.info({ type: envelope.type }, '[company-specialists-service] no handler for this event type - ignoring');
  });
}
