...
export const AUTH_DEAD_QUEUE = 'auth.dead.q';
...
export async function declareTopology(channel: Channel): Promise<void> {
  ...
  await channel.assertQueue(AUTH_DEAD_QUEUE, { durable: true });
  await channel.bindQueue(AUTH_DEAD_QUEUE, DOMAIN_EVENTS_DLX, '#');
}
