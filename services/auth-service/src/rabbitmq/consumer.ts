...
export async function consumeFromRabbitMq(options: ConsumeOptions): Promise<RabbitMqConsumer> {
  ...
  connection.on('error', (err: unknown) => logger.warn({ err }, '[auth-service] RabbitMQ connection error'));
...
async function handleMessage(
  ...
  try {
    ...
    logger.error(
      ...
      '[auth-service] failed to process message - routing to dead-letter queue',
    );
