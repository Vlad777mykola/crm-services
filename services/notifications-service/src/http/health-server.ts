...
export function startHealthServer(port: number, pool: Pool, consumer: RabbitMqConsumer): http.Server {
  const server = http.createServer((req, res) => {
    ...
    if (req.url === '/health/ready') {
      pool
        ...
        .catch((err: unknown) => {
          logger.warn({ err }, '[notifications-service] readiness check failed');
  ...
  server.listen(port, () => {
    logger.info(`[notifications-service] health server listening on :${port} (/health/live, /health/ready)`);
  });
