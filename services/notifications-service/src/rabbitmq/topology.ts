...
export const NOTIFICATIONS_DEAD_QUEUE = 'notifications.dead.q';
...
export async function declareTopology(channel: Channel): Promise<void> {
  ...
  await channel.assertQueue(NOTIFICATIONS_DEAD_QUEUE, { durable: true });
  await channel.bindQueue(NOTIFICATIONS_DEAD_QUEUE, DOMAIN_EVENTS_DLX, '#');
}
