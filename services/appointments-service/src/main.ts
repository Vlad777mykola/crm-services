...
async function bootstrap(): Promise<void> {
  ...
  if (env.AUTO_DDL) {
    logger.warn('[appointments-service] AUTO_DDL is enabled; use only for temporary local compatibility');
  ...
  const consumer = await consumeFromRabbitMq({
    ...
    bindings: [
      ...
      { exchange: DOMAIN_EVENTS_EXCHANGE, routingKey: 'specialist.created' },
      { exchange: DOMAIN_EVENTS_EXCHANGE, routingKey: 'specialist.updated' },
