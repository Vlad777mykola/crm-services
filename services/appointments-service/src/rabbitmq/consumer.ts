...
export async function consumeFromRabbitMq(options: ConsumeOptions): Promise<RabbitMqConsumer> {
  ...
  connection.on('error', (err: unknown) => logger.warn({ err }, '[appointments-service] RabbitMQ connection error'));
...
async function handleMessage(
  ...
  try {
    ...
    logger.error(
      ...
      '[appointments-service] failed to process message - routing to dead-letter queue',
    );
