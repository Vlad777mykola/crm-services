...
export const COMPANY_MEMBERS_DEAD_QUEUE = 'company-members.dead.q';
...
export async function declareTopology(channel: Channel): Promise<void> {
  ...
  await channel.assertQueue(COMPANY_MEMBERS_DEAD_QUEUE, { durable: true });
  await channel.bindQueue(COMPANY_MEMBERS_DEAD_QUEUE, DOMAIN_EVENTS_DLX, '#');
}
