...
| `processed_events` | not yet in main DB (pattern used by `services/*` workers) | each consumer service owns its own | per-service schema | every phase with a consumer | n/a | n/a | None | Already the standard for `notifications-service`, `ai-service`, and every extracted service with a consumer; new services copy the same shape: `(event_id, consumer_name, processed_at)`. |
| `appointment_recommendation_projections` | `services/appointments-service/src/db/schema.ts` | **Resolved (Phase 12): appointments-service** | `appointments_schema` | 12 | Backend API read-only (no route yet) | appointments-service only | None — old backend-projection-service table dropped along with the service, rebuilt from new `ai.appointment_recommendation_created` events | AI-derived projection, not a source-of-truth table; safe to drop and rebuild. |
| `company_insight_projections` | `services/companies-service/src/db/schema.ts` | **Resolved (Phase 12): companies-service** | `companies_schema` | 12 | Backend API read-only (no route yet) | companies-service only | None — same as above, rebuilt from `ai.company_insight_created` | Same category as above. |
...
| `appointment_recommendation_projections`, `company_insight_projections` future owner | Phase 12 | Move to appointments-service / companies-service vs. keep on backend-projection-service indefinitely | **Resolved:** moved to appointments-service and companies-service respectively; `backend-projection-service` retired |
...
No table in the main Postgres database has an unknown future owner. As of
Phase 12, the two former `backend-projection-service` projection tables also
have confirmed owners (appointments-service, companies-service) — every
table in this document is now resolved except the `service_specialists`
ownership question, which is explicitly tracked above rather than silently
assumed.
